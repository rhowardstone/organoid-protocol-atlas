...
paper (PMC)
  ...
  └─ REST API (Datasette plugin, 30 routes)
```
...
serve/
  ...
  plugins/
    analytics_endpoint.py    30-route analytics REST API (pure handlers + thin Datasette wrappers)
...
tests/                       offline test suite (883 tests, no network, no GPU)
...
make test                               # run offline test suite (883 tests)
