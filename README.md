...
paper (PMC)
  ...
  └─ REST API (Datasette plugin, 43 routes)
```
...
serve/
  ...
  plugins/
    analytics_endpoint.py    43-route analytics REST API (pure handlers + thin Datasette wrappers)
...
make test                               # run offline test suite (1053 tests)
