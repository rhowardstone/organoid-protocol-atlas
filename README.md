...
paper (PMC)
  ...
  └─ REST API (Datasette plugin, 40 routes)
```
...
serve/
  ...
  plugins/
    analytics_endpoint.py    40-route analytics REST API (pure handlers + thin Datasette wrappers)
...
make test                               # run offline test suite (1014 tests)
