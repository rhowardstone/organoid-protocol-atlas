...
paper (PMC)
  ...
  └─ REST API (Datasette plugin, 31 routes)
```
...
serve/
  ...
  plugins/
    analytics_endpoint.py    31-route analytics REST API (pure handlers + thin Datasette wrappers)
...
tests/                       offline test suite (893 tests, no network, no GPU)
...
make test                               # run offline test suite (893 tests)
