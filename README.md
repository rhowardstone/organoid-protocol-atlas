...
paper (PMC)
  ...
  └─ REST API (Datasette plugin, 44 routes)
```
...
serve/
  ...
  plugins/
    analytics_endpoint.py    44-route analytics REST API (pure handlers + thin Datasette wrappers)
...
make test                               # run offline test suite (1066 tests)
